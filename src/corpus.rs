...
pub fn create_corpus_dir(base: impl AsRef<Path>, target: &str) -> Result<PathBuf> {
    ...
    fs::create_dir_all(&corpus_dir).context(format!("unable to create corpus dir for {}{}", base.display(), target))?;
    ...
    match target {
        ...
        "util_to_hex" => {
            while idx < num_items {
                ...
                let filename = corpus_dir.join(name);
                let mut f = fs::File::create(&filename).with_context(|| format!("Failed to create file: {:?}", filename))?;
                f.write_all(&seeds.as_bytes()).with_context(|| format!("Failed to write to file: {:?}", filename))?;
                idx += 1;
            }
           Ok(corpus_dir)
        },
        ...
        "crypto_opcodes_to_hash"  => {
            while idx < num_items {
                ...
                let filename = corpus_dir.join(name);
                let mut f = fs::File::create(&filename).with_context(|| format!("Failed to create file: {:?}", filename))?;
                f.write_all(seeds_payload).with_context(|| format!("Failed to write to file: {:?}", filename))?;
                idx += 1;
            }
           Ok(corpus_dir)
